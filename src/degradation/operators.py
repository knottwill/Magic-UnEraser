...
class Eraser(DegredationOperator):
    ...
    @details This operator applies a mask to the input image, with severity controlled by the variable `s`.
    The idea is that the operator simulates the effect of an eraser 'rubbing out the image', where the eraser head
    is moved across the image in a zigzag pattern. The mask is generated by a series of individual Eraser head masks,
    ...
    leaves behind a small amount of noise as it moves across the image. This noise is gaussian with standard deviation 0.02.
    """
    def __init__(self, trajectory, eraserhead_radius: int = 3, sigma: float = 3, size: int = 28):
        super().__init__()
